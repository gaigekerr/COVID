...
def extract_nuts_era5(year, vnuts, focus_countries=None):
    ...
    gpw = gpw+mask 
    # Restrict to the quasi-European domain (similar to above)
    ...
    # d2m
    d2ml_df.index.name = 'NUTS'
    d2ml_df.to_csv(DIR_OUT+'ERA5_NUTS%d_d2m_%s_%s.csv'
        %(vnuts,pd.to_datetime(d2ml_dates[0]).strftime('%Y%m%d'),
          pd.to_datetime(d2ml_dates[-1]).strftime('%Y%m%d')), sep=',')
    d2ml_df_wt.index.name = 'NUTS'
    d2ml_df_wt.to_csv(DIR_OUT+'ERA5_NUTS%d_d2m_%s_%s_popwtd.csv'
        %(vnuts,pd.to_datetime(d2ml_dates[0]).strftime('%Y%m%d'),
          pd.to_datetime(d2ml_dates[-1]).strftime('%Y%m%d')), sep=',')
    # pev
    pevl_df.index.name = 'NUTS'
    pevl_df.to_csv(DIR_OUT+'ERA5_NUTS%d_pev_%s_%s.csv'
        %(vnuts,pd.to_datetime(pevl_dates[0]).strftime('%Y%m%d'),
          pd.to_datetime(pevl_dates[-1]).strftime('%Y%m%d')), sep=',')
    pevl_df_wt.index.name = 'NUTS'
    pevl_df_wt.to_csv(DIR_OUT+'ERA5_NUTS%d_pev_%s_%s_popwtd.csv'
        %(vnuts,pd.to_datetime(pevl_dates[0]).strftime('%Y%m%d'),
          pd.to_datetime(pevl_dates[-1]).strftime('%Y%m%d')), sep=',')
    # sp
    spl_df.index.name = 'NUTS'
    spl_df.to_csv(DIR_OUT+'ERA5_NUTS%d_sp_%s_%s.csv'
        %(vnuts,pd.to_datetime(spl_dates[0]).strftime('%Y%m%d'),
          pd.to_datetime(spl_dates[-1]).strftime('%Y%m%d')), sep=',')
    spl_df_wt.index.name = 'NUTS'
    spl_df_wt.to_csv(DIR_OUT+'ERA5_NUTS%d_sp_%s_%s_popwtd.csv'
        %(vnuts,pd.to_datetime(spl_dates[0]).strftime('%Y%m%d'),
          pd.to_datetime(spl_dates[-1]).strftime('%Y%m%d')), sep=',')
    # ssrd
    ssrdl_df.index.name = 'NUTS'
    ssrdl_df.to_csv(DIR_OUT+'ERA5_NUTS%d_ssrd_%s_%s.csv'
        %(vnuts,pd.to_datetime(ssrdl_dates[0]).strftime('%Y%m%d'),
          pd.to_datetime(ssrdl_dates[-1]).strftime('%Y%m%d')), sep=',')
    ssrdl_df_wt.index.name = 'NUTS'
    ssrdl_df_wt.to_csv(DIR_OUT+'ERA5_NUTS%d_ssrd_%s_%s_popwtd.csv'
        %(vnuts,pd.to_datetime(ssrdl_dates[0]).strftime('%Y%m%d'),
          pd.to_datetime(ssrdl_dates[-1]).strftime('%Y%m%d')), sep=',')
    # swvl1
    swvl1l_df.index.name = 'NUTS'
    swvl1l_df.to_csv(DIR_OUT+'ERA5_NUTS%d_swvl1_%s_%s.csv'
        %(vnuts,pd.to_datetime(swvl1l_dates[0]).strftime('%Y%m%d'),
          pd.to_datetime(swvl1l_dates[-1]).strftime('%Y%m%d')), sep=',')
    swvl1l_df_wt.index.name = 'NUTS'
    swvl1l_df_wt.to_csv(DIR_OUT+'ERA5_NUTS%d_swvl1_%s_%s_popwtd.csv'
        %(vnuts,pd.to_datetime(swvl1l_dates[0]).strftime('%Y%m%d'),
          pd.to_datetime(swvl1l_dates[-1]).strftime('%Y%m%d')), sep=',')
    # swvl2
    swvl2l_df.index.name = 'NUTS'
    swvl2l_df.to_csv(DIR_OUT+'ERA5_NUTS%d_swvl2_%s_%s.csv'
        %(vnuts,pd.to_datetime(swvl2l_dates[0]).strftime('%Y%m%d'),
          pd.to_datetime(swvl2l_dates[-1]).strftime('%Y%m%d')), sep=',')
    swvl2l_df_wt.index.name = 'NUTS'
    swvl2l_df_wt.to_csv(DIR_OUT+'ERA5_NUTS%d_swvl2_%s_%s_popwtd.csv'
        %(vnuts,pd.to_datetime(swvl2l_dates[0]).strftime('%Y%m%d'),
          pd.to_datetime(swvl2l_dates[-1]).strftime('%Y%m%d')), sep=',')    
    # swvl3
    swvl3l_df.index.name = 'NUTS'
    swvl3l_df.to_csv(DIR_OUT+'ERA5_NUTS%d_swvl3_%s_%s.csv'
        %(vnuts,pd.to_datetime(swvl3l_dates[0]).strftime('%Y%m%d'),
          pd.to_datetime(swvl3l_dates[-1]).strftime('%Y%m%d')), sep=',')
    swvl3l_df_wt.index.name = 'NUTS'
    swvl3l_df_wt.to_csv(DIR_OUT+'ERA5_NUTS%d_swvl3_%s_%s_popwtd.csv'
        %(vnuts,pd.to_datetime(swvl3l_dates[0]).strftime('%Y%m%d'),
          pd.to_datetime(swvl3l_dates[-1]).strftime('%Y%m%d')), sep=',')
    # swvl4
    swvl4l_df.index.name = 'NUTS'
    swvl4l_df.to_csv(DIR_OUT+'ERA5_NUTS%d_swvl4_%s_%s.csv'
        %(vnuts,pd.to_datetime(swvl4l_dates[0]).strftime('%Y%m%d'),
          pd.to_datetime(swvl4l_dates[-1]).strftime('%Y%m%d')), sep=',')
    swvl4l_df_wt.index.name = 'NUTS'
    swvl4l_df_wt.to_csv(DIR_OUT+'ERA5_NUTS%d_swvl4_%s_%s_popwtd.csv'
        %(vnuts,pd.to_datetime(swvl4l_dates[0]).strftime('%Y%m%d'),
          pd.to_datetime(swvl4l_dates[-1]).strftime('%Y%m%d')), sep=',')
    # t2mmax
    t2mmaxl_df.index.name = 'NUTS'
    t2mmaxl_df.to_csv(DIR_OUT+'ERA5_NUTS%d_t2mmax_%s_%s.csv'
        %(vnuts,pd.to_datetime(t2mmaxl_dates[0]).strftime('%Y%m%d'),
          pd.to_datetime(t2mmaxl_dates[-1]).strftime('%Y%m%d')), sep=',')
    t2mmaxl_df_wt.index.name = 'NUTS'
    t2mmaxl_df_wt.to_csv(DIR_OUT+'ERA5_NUTS%d_t2mmax_%s_%s_popwtd.csv'
        %(vnuts,pd.to_datetime(t2mmaxl_dates[0]).strftime('%Y%m%d'),
          pd.to_datetime(t2mmaxl_dates[-1]).strftime('%Y%m%d')), sep=',')
    # t2mmin
    t2mminl_df.index.name = 'NUTS'
    t2mminl_df.to_csv(DIR_OUT+'ERA5_NUTS%d_t2mmin_%s_%s.csv'
        %(vnuts,pd.to_datetime(t2mminl_dates[0]).strftime('%Y%m%d'),
          pd.to_datetime(t2mminl_dates[-1]).strftime('%Y%m%d')), sep=',')
    t2mminl_df_wt.index.name = 'NUTS'
    t2mminl_df_wt.to_csv(DIR_OUT+'ERA5_NUTS%d_t2mmin_%s_%s_popwtd.csv'
        %(vnuts,pd.to_datetime(t2mminl_dates[0]).strftime('%Y%m%d'),
          pd.to_datetime(t2mminl_dates[-1]).strftime('%Y%m%d')), sep=',')
    # t2mavg
    t2mavgl_df.index.name = 'NUTS'
    t2mavgl_df.to_csv(DIR_OUT+'ERA5_NUTS%d_t2mavg_%s_%s.csv'
        %(vnuts,pd.to_datetime(t2mavgl_dates[0]).strftime('%Y%m%d'),
          pd.to_datetime(t2mavgl_dates[-1]).strftime('%Y%m%d')), sep=',')
    t2mavgl_df_wt.index.name = 'NUTS'
    t2mavgl_df_wt.to_csv(DIR_OUT+'ERA5_NUTS%d_t2mavg_%s_%s_popwtd.csv'
        %(vnuts,pd.to_datetime(t2mavgl_dates[0]).strftime('%Y%m%d'),
          pd.to_datetime(t2mavgl_dates[-1]).strftime('%Y%m%d')), sep=',')
    # tp
    tpl_df.index.name = 'NUTS'
    tpl_df.to_csv(DIR_OUT+'ERA5_NUTS%d_tp_%s_%s.csv'
        %(vnuts,pd.to_datetime(tpl_dates[0]).strftime('%Y%m%d'),
          pd.to_datetime(tpl_dates[-1]).strftime('%Y%m%d')), sep=',')
    tpl_df_wt.index.name = 'NUTS'
    tpl_df_wt.to_csv(DIR_OUT+'ERA5_NUTS%d_tp_%s_%s_popwtd.csv'
        %(vnuts,pd.to_datetime(tpl_dates[0]).strftime('%Y%m%d'),
          pd.to_datetime(tpl_dates[-1]).strftime('%Y%m%d')), sep=',')    
    # u10
    u10l_df.index.name = 'NUTS'
    u10l_df.to_csv(DIR_OUT+'ERA5_NUTS%d_u10_%s_%s.csv'
        %(vnuts,pd.to_datetime(u10l_dates[0]).strftime('%Y%m%d'),
          pd.to_datetime(u10l_dates[-1]).strftime('%Y%m%d')), sep=',')
    u10l_df_wt.index.name = 'NUTS'
    u10l_df_wt.to_csv(DIR_OUT+'ERA5_NUTS%d_u10_%s_%s_popwtd.csv'
        %(vnuts,pd.to_datetime(u10l_dates[0]).strftime('%Y%m%d'),
          pd.to_datetime(u10l_dates[-1]).strftime('%Y%m%d')), sep=',')
    # v10
    v10l_df.index.name = 'NUTS'
    v10l_df.to_csv(DIR_OUT+'ERA5_NUTS%d_v10_%s_%s.csv'
        %(vnuts,pd.to_datetime(v10l_dates[0]).strftime('%Y%m%d'),
          pd.to_datetime(v10l_dates[-1]).strftime('%Y%m%d')), sep=',')
    v10l_df_wt.index.name = 'NUTS'
    v10l_df_wt.to_csv(DIR_OUT+'ERA5_NUTS%d_v10_%s_%s_popwtd.csv'
        %(vnuts,pd.to_datetime(v10l_dates[0]).strftime('%Y%m%d'),
          pd.to_datetime(v10l_dates[-1]).strftime('%Y%m%d')), sep=',')
    # slhf
    slhfl_df.index.name = 'NUTS'
    slhfl_df.to_csv(DIR_OUT+'ERA5_NUTS%d_slhf_%s_%s.csv'
        %(vnuts,pd.to_datetime(slhfl_dates[0]).strftime('%Y%m%d'),
          pd.to_datetime(slhfl_dates[-1]).strftime('%Y%m%d')), sep=',')
    slhfl_df_wt.index.name = 'NUTS'
    slhfl_df_wt.to_csv(DIR_OUT+'ERA5_NUTS%d_slhf_%s_%s_popwtd.csv'
        %(vnuts,pd.to_datetime(slhfl_dates[0]).strftime('%Y%m%d'),
          pd.to_datetime(slhfl_dates[-1]).strftime('%Y%m%d')), sep=',')    
    return 
...
def extract_ibge_era5(var, year, adminlev):
    ...
    gpw = np.hstack([gpw[:,720:], gpw[:,:720]])
    gpw = gpw+mask 
    # Restrict to the quasi-Brazil domain (similar to above)
    ...
    df_wt = pd.DataFrame(columns=[x[:-9] for x in dates])    
    ...
    for ishape in np.arange(0, len(shapes), 1):
        ...
        if len(i_inside)!=0:
            ...
            gpw_admin = gpw[i_inside, j_inside]
            with warnings.catch_warnings():
                warnings.simplefilter('ignore')
                mask = np.where((gpw_admin >= 0.0) & (
                    np.isreal(varl_admin[0,:]) == True))[0]
                # Total population in unit 
                sumpop = np.nansum(gpw_admin[mask])
                varl_admin_wt = varl_admin[:,mask]*np.broadcast_to(gpw_admin[mask],(
                    np.shape(varl_admin[:,mask])))/sumpop
                # Conduct population-weighted spatial average over region
                if sumpop > 0.0:
                    varl_admin_wt = np.nansum(varl_admin_wt, axis=1)
                else:
                    varl_admin_wt = np.empty((len(varl)))
                    varl_admin_wt[:] = np.nan                    
                # Conduct simple spatial average over region (i.e., no population
                # weighting)
                varl_admin = np.nanmean(varl_admin, axis=1)  
        else: 
            varl_admin = np.empty((len(varl)))
            varl_admin_wt = np.empty((len(varl)))
            varl_admin[:] = np.nan
            varl_admin_wt[:] = np.nan
        # Add row corresponding to individual unit for unweighted
        row_df = pd.DataFrame([varl_admin], index=[gid], 
            columns=[x[:-9] for x in dates])
        df = pd.concat([row_df, df])
        # For weighted
        row_df_wt = pd.DataFrame([varl_admin_wt], index=[gid], 
            columns=[x[:-9] for x in dates])
        df_wt = pd.concat([row_df_wt, df_wt])
        # Update progress bar
        percent = 100.0*ishape/total  
        sys.stdout.write('\r')
        sys.stdout.write("Completed: [{:{}}] {:>3}%".format('='*int(
    ...
    df_wt.index.name = 'IBGE'
    df_wt.to_csv(DIR_OUT+'ERA5_IBGE%d_%s_%s_%s_popwtd.csv'
        %(adminlev,var,pd.to_datetime(dates[0]).strftime('%Y%m%d'),
          pd.to_datetime(dates[-1]).strftime('%Y%m%d')), sep=',')    
    return
